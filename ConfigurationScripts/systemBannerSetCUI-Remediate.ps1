...
try {
    ...
    Write-Host "SystemBanner CUI configuration failed: $($_.Exception.Message)"
    exit 1
}
...
if ($null -eq $Simple -or $Simple.Simple -ne 2) {
    Write-Host "Validation failed: Simple Classification is not set to CUI."
    exit 1
}
...
Write-Host "SystemBanner is correctly configured for CUI, TOP ONLY and has been started."
exit 0